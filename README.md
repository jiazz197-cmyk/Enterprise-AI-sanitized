...
Yamato AI 助手平台是为**大和衡器（上海）**量身定制的企业内部 AI 工作台，仅服务于大和自身的业务运转，**不面向外部部署**。它把原本散落在图纸、老 ERP 系统、Excel 与员工经验里的报价与订单流程，收敛到一个对话式工作台：员工用自然语言提问、上传 PDF 图纸即可自动生成报价、填写报单，AI 在背后对接大和既有的 PDM 与 U8（用友）ERP 系统完成数据回填与校核。
